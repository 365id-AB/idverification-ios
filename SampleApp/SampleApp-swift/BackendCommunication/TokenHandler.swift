...
public struct TokenHandler: Codable {
    ...
    private mutating func setExpireTimeTxt() {
        guard let expireTime = Calendar.current.date(
            byAdding: .second,
            value: self.expiresIn ?? 0,
            to: Date()
        ) else { return }
    ...
    private func getTokenSecondsLeft() -> Int {
    ...
    private static func jsonToTokenData(_ json: String) -> TokenHandler? {
        guard let data: Data = json.data(using: .utf8) else { return nil }
        return try? JSONDecoder().decode(TokenHandler.self, from: data)
    }

    /** Create json from TokenData object */
    private static func tokenDataToJson(_ tokenData: TokenHandler) -> String? {
        guard let jsonData = try? JSONEncoder().encode(tokenData) else { return nil }
        return String(data: jsonData, encoding: .utf8)
    }

    /** Save the TokenData on the device */
    private static func save(clientSecret: String, clientId: String, responseBody: String) -> Bool {
    ...
    private static func load() -> TokenHandler? {
    ...
    private static func deleteIfCredentialsChanged(clientSecret: String, clientId: String) {
        guard let tokenData = TokenHandler.load() else { return }
    ...
    public static func getToken(
        ...
        TokenHandler.deleteIfCredentialsChanged(clientSecret: clientSecret, clientId: clientId)
        let storedTokenData = TokenHandler.load()
        ...
        runRequest(request: request, clientSecret: clientSecret, clientId: clientId, completionHandler)
    }

    /** Run the request to get a new token */
    private static func runRequest(
        ...
        URLSession.shared.dataTask(with: request) { [weak mainThread, weak currentThread] (data, response, error) in
            ...
            guard
                ...
                TokenHandler.save(clientSecret: clientSecret, clientId: clientId, responseBody: responseBody)
            else {
            ...
            mainThread?.async {
                completionHandler(TokenHandler.load()?.getAccessToken() ?? nil)
            }
        }.resume()
    }
}
